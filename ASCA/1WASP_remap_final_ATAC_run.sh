...
#for cell in Rod
#for cell in OFFBC ONBC Astro Rod MG Cone RGC HC AC
for cell in OFFBC ONBC Rod MG Cone RGC HC AC
...
do
for region in Lobe Macular
do
race=$2
label=${indi}_${region}_${cell}
...
bam="sc_human_retina/data/single_cell/snATAC/lobe_macular/out-cluster-ATAC_"${indi}"_"${cell}
...
#####sh sc_human_retina/scripts/ASE/WASP_remap_final_ATAC_phased.sh  $label $indi  $race
sh sc_human_retina/scripts/ASE/WASP_remap_final_ATAC_phased20ppl_hg19.sh  $label $indi  $race
####sh sc_human_retina/scripts/ASE/WASP_remap_final_ATAC_phased20ppl_hg19_rest.sh  $label $indi  $race
done
done
done < $file
